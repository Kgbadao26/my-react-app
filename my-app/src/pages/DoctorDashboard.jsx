...
export default function DoctorDashboard() {
  ...
  const stats = {
    ...
    upcoming:  appointments.filter(a => {
      if (a.status === 'cancelled' || a.status === 'expired') return false;
      // Compare against the actual appointment time, not just midnight of
      // the date — same date+time parsing (and same server/local timezone
      // caveat) as the expiry cron job in server.js.
      const apptDateTime = a.time ? new Date(`${a.date}T${a.time}:00`) : new Date(a.date);
